...
const PopoverStyle = styled('div')`
  ...
  ul, li, a {
    opacity: ${props => props.height ? 1 : 0};
    width: ${props => props.width}px;
    height: ${props => props.height / (props.length + 1)}px;
    white-space: nowrap;
...
class Popover extends Component {
  ...
  componentDidUpdate = (prevProps) => {
    if (!prevProps.open && this.props.open) {
      const height = this.props.length * 60;
  ...
  render() {
    return (
      <PopoverStyle isOpen={this.props.open} height={this.state.height} width={this.state.width} length={this.props.length}>
